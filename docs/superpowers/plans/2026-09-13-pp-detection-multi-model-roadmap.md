...
- **多目标跟踪：0.1.0 发布候选，等待远程交付核验。** 纯算法标准 1.2.0、ByteTrack 高低分思想的独立数学实现、CPU/main、有状态 API、双语 Demo 及 Vanilla/React 示例已完成，代码 Apache-2.0，不复制或分发旧参考实现。[前期评估](../../../reports/tracking/2026-09-18-feasibility/README.md)和[原本地验收](../../../reports/tracking/2026-09-19-foundation/README.md)保持历史原貌。新增固定 MOT17 七段 FRCNN 训练序列 5316 帧：默认 IDF1 48.2922%、IDSW 1101、MOTA 44.4010%、FP 4169、FN 57166；低分消融 IDF1 48.3465%、IDSW 1066，说明低分关联不是普遍精度提升。这不是测试集、官方 ByteTrack 排名或视频端到端速度。门户本地准备第八个纯算法条目，远程合并等待 npm、Release、HTTPS Demo 和治理证据；尚不声称线上发布。移动设备及其他后端仍未验证，Workflow 继续暂缓。
