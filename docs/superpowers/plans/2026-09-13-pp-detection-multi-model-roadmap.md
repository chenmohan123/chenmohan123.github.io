...
下一阶段先统一精度变体的发布口径：以保持目标识别为目的时，六变体均通过历史 S/PicoDet 的单轮质量口径，有继续验证价值；要求坐标接近 FP32 的应用继续使用 FP32。口径应在下一批验证前明确，然后完成三轮复现、Worker 生命周期、ModelScope/Hugging Face 固定分发回读与 Demo 验证。体积收益独立计入发布判断，不额外要求推理必须加速。本批次报告位于 SDK 的 `reports/evaluation/2026-09-14-ppyoloe-mlx-precision/README.md`。
