...
任务目录沿用启动日 2026-09-18；补空记录修复后封存运行实际为 **2026-09-19 01:15:07（北京时间）**，即 `2026-09-18T17:15:07.507523+00:00`。共运行五种配置、十类输入及两个适配对照，合计 **60 组、860 次 update（775次源帧调用、85次补空调用）**，另有五组多实例诊断。所有场景均成功执行；这里的“执行成功”包含预期记录的身份交换、遗漏与全局状态失败，不表示跟踪质量全部达标。
...
| `results.json.gz` | 源帧框、ID、状态、耗时和 subject 轨迹，以及860次真实调用的独立输入/输出/状态记录（含85次补空）；gzip 使用 `mtime=0` |
...
| `verify_calls.py` | 独立从输入展开调用序列，检查覆盖、连续补空帧序、输入布局/空输入、源帧聚合和补空统计；含五项污染负例 |
...
该命令先核对证据哈希、固定源码、确定性输入、包版本和 `pip check`，随后运行全部场景，在 `.tmp/tracking-rerun.json` 保留未压缩重跑结果。源帧完整框、ID、状态、评分轨迹、计数、配置，以及每次真实调用的 `call_records` 全部比较，浮点容差 `rtol=atol=1e-9`；只剔除耗时、运行时间戳和环境描述等非确定性字段（依赖版本另行严格核验）。每条调用记录包含场景内 `call_index`、本类别实例内 `group_call_index`（均从1开始）、实际 `frame_index`、所属 `source_frame_index`、`is_padding`、`class_group`、传入核心的数组形状/类型/值、类别前缀适配前的输出，以及调用后的原始状态。补空帧3–19归属下一源帧20；同一类别内调用顺序保持真实执行顺序，不对状态列表去重。任何哈希、版本或输出不符均非零退出。摘要还须与压缩证据逐项相符。2026-09-19 修复后实际重跑通过。

独立检查补空覆盖及污染负例：

```powershell
.tmp/tracking-venv/Scripts/python.exe reports/tracking/2026-09-18-feasibility/verify_calls.py --negative-checks
```

该检查不导入运行器的构造逻辑，独立从 `inputs.json` 展开预期调用，核对860条记录、85条补空、五组各17条帧3–19连续补空、实际空数组及零观测输出；源帧聚合须与原始调用一致。五种内存污染（删记录、错帧序、非空补空输入、伪造输出、去重 Byte removed）均须被拒绝。修复前的旧归档在新校验器下因缺少逐次记录返回1；修复后覆盖与五项负例通过。
...
- 指标 `id_changes` 是每个 subject 在相邻非空匹配之间的 ID 变化次数，含长丢失后合理的新身份；不是标准 MOT IDSW/IDF1/HOTA。`unmatched_detection_frames` 只计有检测但未匹配输出的目标帧，空检测帧不进入该分母。完全重叠帧从评分排除，完整输出仍保留。`empty_input_frames/outputs_on_empty_input` 只统计原始源帧，不能用来证明补空表现；新增 `summary.padding.calls/empty_input_calls/output_count` 只统计补空调用，五组各为17/17/0，其余组为0/0/0。不能把“漏检输入帧未计入分母”解读为恢复了检测。
...
补空逐次状态证据还明确：官方 Byte 在稀疏序列的补空帧3及7仍为 `lost=[1], removed=[]`；帧8为 `lost=[1], removed=[1]`；帧9起为 `lost=[], removed=[1,1]`。重复项属于固定参考的原始行为，记录与校验均不去重。五种配置85次补空均无观测输出；这个结论由逐次调用记录证明，与源帧空输入统计分开。
...
| --- | ---: | ---: | ---: | ---: |
| Byte | 51.4596 | 0.3225 | 0.4763 | 1.3179 |
| OC 基础 | 92.4383 | 0.5451 | 0.9799 | 1.4434 |
| OC+Byte | 95.4682 | 0.5395 | 0.9567 | 2.2722 |
| Paddle Byte | 81.2500 | 0.4964 | 0.8427 | 1.1477 |
| Paddle OC+Byte | 69.4311 | 0.3902 | 0.6968 | 1.7710 |
