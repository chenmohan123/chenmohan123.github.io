...
> 2026-09-26 回填归档：本文件与文中引用的 `../2026-09-18-feasibility/`、
> `reports/sdk-standard/segmentation-{before,after}.json` 此前遗留在已关闭未合并的本地分支
> `codex/independent-sdk-priority`（PR #53 被 #54 取代），现取回门户唯一副本。
> 文中"严格掩码门槛仍为 **failed**"是当日阶段口径；该尺寸语义随后改按原图整数尺寸独立参考验收，
> 现状以 `src/content/models/pp-segmentation.yaml` 的 limitations 与
> `reports/segmentation/2026-09-18-portal-registry/` 为准。
