...
export default function ModelDirectory({ models }: { models: ModelData[] }) {
  ...
  return <section aria-label="模型目录">
    <div className="directory-toolbar">
      ...
      <select aria-label="任务" value={filters.task} onChange={(event) => set('task', event.target.value as ModelFilters['task'])}><option value="all">全部任务</option><option value="document-layout">文档版面</option><option value="ocr">OCR</option><option value="detection">检测</option><option value="pose-estimation">{taskDisplayNames['pose-estimation']}</option><option value="instance-segmentation">{taskDisplayNames['instance-segmentation']}</option><option value="asr">ASR</option><option value="tts">TTS</option><option value="image-correction">图像矫正</option><option value="vision-language">视觉语言</option></select>
