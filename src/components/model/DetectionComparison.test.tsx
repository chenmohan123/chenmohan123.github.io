...
describe('Detection 模型选型', () => {
  it('筛选稳定精度并保留批次，不把XS实验量化列入稳定表', () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent('37 个稳定变体');
    fireEvent.change(screen.getByLabelText('模型系列'), { target: { value: 'picodet' } });
    fireEvent.change(screen.getByLabelText('模型精度'), { target: { value: 'w8a32' } });
    expect(screen.getByRole('status')).toHaveTextContent('7 个稳定变体');
    expect(screen.queryByRole('row', { name: /PicoDet-XS/ })).not.toBeInTheDocument();
  ...
  it('切换后端同时更新AP与耗时，明确不同汇总规则', () => {
    ...
    expect(screen.getAllByText(/再取三轮中位数/)).toHaveLength(2);
  });
});
