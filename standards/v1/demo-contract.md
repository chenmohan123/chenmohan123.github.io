...
## 算法与可选模型混合 Demo（1.3.0）

混合 Demo 仍聚焦一个 SDK，在同一工作台中提供默认算法流程和显式启用的模型
增强能力。页面同时保留 `data-sdk-algorithm-info`、`data-sdk-model-info`、
`data-sdk-runtime-info` 和 `data-sdk-timing`，并分别提供
`data-sdk-state-reset` 与模型缓存清理控件。

未启用模型时不得请求模型 runtime 或权重。启用模型后分别显示两个模块的
请求/实际后端、执行模式和分层耗时；不能把模型 WebGPU 标签用于 CPU 算法。
模型清理和状态复位是不同动作；完整清理会取消提取并复位依赖模型特征的关联
状态。切换语言仍不得改变任一模块状态，静态标记也不能替代浏览器交互验证。
