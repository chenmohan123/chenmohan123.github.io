...
日期：2026-09-20。状态：**提案，未生效**。当前标准仍为1.2.0；schema、rules、checker及生产manifest均未增加hybrid类型。本文件是后续实现依据，不能用它提前声称合规或绕过当前规则。
...
在实际实施时同时更新README中英入口、sdk/runtime/demo/performance/docs契约、rules.yaml、schema、检查器、模板和回归测试：
...
- `verification.environments`后续可增加可选`module`枚举algorithm/model，hybrid必须对每模块提供至少一项日期证据；纯model/algorithm旧条目不受影响。禁止用特性探测替代实际模型执行。
