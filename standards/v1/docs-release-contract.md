...
混合 SDK 同时记录算法来源、状态生命周期和输入输出契约，以及模型身份、资产、
许可、精度、不可变来源和缓存行为。文档必须区分默认算法入口与可选模型子入口，
说明未导入模型时不会加载推理引擎或权重，并分别描述状态复位、模型清理、取消
和释放语义。

发布说明列出两个 `package.exports` 入口、两个模块的后端与耗时、每模块带日期
的兼容性证据和已知限制。研究阶段的本地文件或 ArrayBuffer 结果不能代替公开
模型分发、hash 和 revision 证据；真实来源尚未准备好时不得发布为合规 hybrid。
